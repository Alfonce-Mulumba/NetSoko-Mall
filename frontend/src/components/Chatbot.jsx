      ...
      {open && (
        <div className="fixed bottom-20 right-6 w-80 bg-white border shadow-lg rounded-lg flex flex-col">
          ...
          <div className="flex-grow p-3 overflow-y-auto h-64">
            {messages.map((m, i) => (
              <div key={i} className={`mb-2 ${m.from === "bot" ? "text-left" : "text-right"}`}>
                <span className={`px-3 py-2 rounded-lg ${m.from === "bot" ? "bg-gray-200" : "bg-green-200"}`}>
                  {m.text}
                </span>
              </div>
            ))}
          ...
          <div className="p-2 border-t flex">
            ...
            />
            <button onClick={handleSend} className="ml-2 bg-primary text-white px-3 py-1 rounded">
              Send
            </button>
          </div>
        </div>
